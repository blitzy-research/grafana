...
- **First run log:** `/tmp/grafana_first_run.log` — **1,356 lines**, 644
  `Executing migration` entries (626 schema + 18 resource-migrator).
- **Second run log:** `/tmp/grafana_second_run.log` — **58 lines** (a 23× smaller
  footprint).
...
| `pkg/services/secrets/kvstore/migrations/migrator.go` | Lines 68–85 (`SecretMigrationProviderImpl.Migrate`); wraps the datasource secret migration in `LockExecuteAndRelease` and logs `Server lock for secret migration already exists` at line 82 when concurrent server lock is held |
| `pkg/services/secrets/kvstore/migrations/datasource_mig.go` | Lines 40–106 (`DataSourceSecretMigrationService.Migrate`); reads `secretMigrationStatus` KV entry, short-circuits on `compatible`, otherwise re-encrypts legacy datasource secrets |
| `pkg/services/featuremgmt/registry.go` | 56 entries with `Expression: "true"` |
| `pkg/util/encoding.go` | Lines 52–56 (`EncodePassword`) — authoritative PBKDF2-HMAC-SHA256 implementation: `pbkdf2.Key(password, salt, 10000, 50, sha256.New)`, hex-encoded to 100 chars |
| `pkg/services/user/password.go` | Lines 30–36 (`Password.Hash`) — thin wrapper that delegates to `util.EncodePassword`; the only path by which user passwords are hashed before storage |
| `pkg/infra/serverlock/serverlock.go` | Lines 137–170 (`LockExecuteAndRelease`) — single-attempt server lock based on `server_lock.operation_uid` UNIQUE constraint; used by the secret migration background service |
| `pkg/server/wire_gen.go` | Wire-generated dependency-injection code (≈1,282 lines); regenerated by `go run ./pkg/build/wire/cmd/wire/main.go gen -tags oss ./pkg/server`; file is `.gitignore`d but **mandatory** for `go build` |

### 14.2 Runtime Artifacts Observed

| Artifact | Description |
|---|---|
| `/tmp/grafana_first_run.log` | First-run startup log, 1,356 lines |
| `/tmp/grafana_second_run.log` | Second-run startup log, 58 lines |
