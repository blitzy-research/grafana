...
Separate from the interval tickers above, one **INFO** line is emitted by the SQLite transaction **retry layer** rather than by any timer, so it is enumerated on its own. It appears **only when two goroutines momentarily contend for the single SQLite write lock**, is therefore **non-periodic**, and its exact timing is **contention-dependent (non-deterministic)**. Across the canonical idle runs it was observed at DB-write contention moments: **reliably** during the **startup background-initialization burst** and the **shutdown DB-finalization sequence** (present in every info run), and — because the timing is contention-dependent — **sometimes also at the idle 10-minute cleanup boundary** (t₀+600 s), where the `cleanup` job's writes and the concurrent `plugins.update.checker` result-write momentarily contend. Its appearance at that boundary is **run-dependent**: across two fresh ~12.7-minute info runs it fired at the cleanup boundary in **one** run (iB) and was **absent** there in the **other** (iA) — a direct demonstration that the line is genuinely non-deterministic and bound to *contention*, not to any fixed lifecycle point or interval. All three contention points (startup, the optional cleanup-boundary occurrence, and shutdown) are shown verbatim below.
...
**Observed at shutdown** (both ~12-minute info runs; the lock-retry is the **last** line emitted, ~11 ms after `"Shutdown started"`, while the api-server storage pruner is exiting):
...
**Observed at the idle 10-minute cleanup boundary** (run iB only; **absent** in run iA — the run-dependence that proves non-determinism). Here the wall-clock times are retained so the co-location at the `t₀+600 s` boundary is visible: the `cleanup` job completes at `:31.523`, the lock-retry fires ~5 ms later at `:31.528`, and the `plugins.update.checker` result-write lands at `:31.587` — three writers clustered at the same 10-minute instant, one of which transiently loses the single SQLite write lock and retries (again `retry=0`, so it succeeded on the first retry):
```
logger=cleanup t=2026-07-15T09:15:31.523396188Z level=info msg="Completed cleanup jobs" duration=4.84089ms
logger=sqlstore.transactions t=2026-07-15T09:15:31.528986157Z level=info msg="Database locked, sleeping then retrying" error="database is locked" retry=0 code="database is locked"
logger=plugins.update.checker t=2026-07-15T09:15:31.587841688Z level=info msg="Update check succeeded" duration=29.437964ms
```
For contrast, run iA crossed the identical boundary with **no** lock-retry (its `cleanup` and `plugins.update.checker` fired at `t₀+600 s` without contending), and its only lock-retry appeared at shutdown — establishing that whether this line appears at the cleanup boundary is decided by run-time contention, not by the boundary itself.

**Cause → effect.** At several points a Grafana boot has multiple background components issuing DB writes concurrently: the startup init burst (state-cache init, plugin/registry updates, update-checkers), the shutdown finalization sequence (the api-server `StorageObjectCountTracker` pruner and other teardown writes), and — at the 10-minute boundary — the `cleanup` job overlapping the `plugins.update.checker` result-write. SQLite allows only **one** writer at a time, so whenever two of these overlap, one writer transiently receives `SQLITE_BUSY`/`SQLITE_LOCKED`; Grafana's retry layer catches it, logs this single INFO line, sleeps 10 ms, and succeeds on retry (`retry=0` with no following `retry=1`). Because it is driven by *overlap* rather than a timer, the line appears **once per contention moment** — reliably at startup and shutdown, and *optionally* at the 10-minute cleanup boundary when the cleanup and update-check writes happen to overlap (present in run iB, absent in run iA) — rather than on any fixed interval. This is precisely why the enumeration of *interval-driven* recurring emitters lists it **separately** as a contention-triggered, non-deterministic line. (Timestamps are shown as `<ts>` for the startup and shutdown blocks, where the exact instant is contention-dependent and carries no interval meaning; the cleanup-boundary block retains wall-clock times because the co-location at `t₀+600 s` is itself the evidence there. In every block it is the *ordering* relative to the surrounding lines that the evidence establishes.)
...
- **Contention-triggered INFO line (non-periodic, not a ticker):** `sqlstore.transactions` `"Database locked, sleeping then retrying"` — observed across multiple canonical idle runs at SQLite write-lock contention moments: **reliably** at the startup background-init burst and the shutdown DB-finalization sequence, and — non-deterministically — **sometimes at the idle 10-minute cleanup boundary** (present in one of two fresh ~12.7-minute info runs, absent in the other; see the *Q1-b contention addendum*). It is emitted by the transaction retry layer (`pkg/services/sqlstore/transactions.go:74`), not by an independent periodic timer, so its appearance is bound to run-time contention rather than to any fixed interval or lifecycle point.
