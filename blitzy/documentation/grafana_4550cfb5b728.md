...
2. The fill is applied with the **nullish-coalescing** operator `??`, so it replaces **only** truly-absent pairings (those that resolve to `undefined`):
