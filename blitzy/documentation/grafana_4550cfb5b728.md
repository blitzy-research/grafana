...
- **Natural resolution** (a present series whose value drops below threshold) is detected in `setNextState`: when `oldState == Alerting && currentState.State == Normal`, `newlyResolved` is set (`manager.go:506-508`), and `shouldTakeImage(..., resolved=true)` returns true via its first disjunct, so an image is captured for the resolved transition.
