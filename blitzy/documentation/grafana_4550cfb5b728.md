...
| `func (s *CacheSegmentedTree) updatePeriodically()` | `rule_cache_segmented.go:28-44` | The single background **writer**: snapshots orgIDs under the lock (L31‑35), rebuilds each via `fillOrg` (L36‑41), then `time.Sleep(20 * time.Second)` (L42) |
...
------------------------                         -----------------------
updatePeriodically()  (rule_cache_segmented.go:28-44)
