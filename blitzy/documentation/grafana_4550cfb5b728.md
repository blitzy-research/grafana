...
| Build/capture commit (stamped)         | `f3d1ae504a` (`f3d1ae504a5cc9568a40f921a070e2fe165ddb6f`) — working-branch HEAD at build time; stamped into `main.commit`/`main.buildstamp`, reported by O3 API | `git rev-parse HEAD`              |
...
`blitzy-0de97a14-ced4-4652-9cb9-337689e950e3`, whose HEAD `f3d1ae504a`
(`f3d1ae504a5cc9568a40f921a070e2fe165ddb6f`) is a **descendant** of the source HEAD and is exactly
what the canonical build stamps into the binary as `main.buildBranch` / `main.commit` (see the
build output in the next section). Consequently the runtime API in O3 reports `commit=f3d1ae504a`
and `buildstamp=1784019135` (the commit-time of that working HEAD), while the product `version`
...
Version: 11.5.0, Linux Version: 11.5.0, Package Iteration: 1784025827pre
rm -r dist
rm -r tmp
...
go build -ldflags -w -X main.version=11.5.0-pre -X main.commit=f3d1ae504a -X main.buildstamp=1784019135 -X main.buildBranch=blitzy-0de97a14-ced4-4652-9cb9-337689e950e3 -o ./bin/linux-amd64/grafana ./pkg/cmd/grafana
go version
go version go1.23.1 linux/amd64
...
```text
go build -ldflags -w -X main.version=11.5.0-pre -X main.commit=f3d1ae504a \
  -X main.buildstamp=1784019135 -X main.buildBranch=blitzy-0de97a14-ced4-4652-9cb9-337689e950e3 \
...
`f3d1ae504a`) and left **completely idle (zero user requests)** for **> 22 minutes** each —
...
- `GET /api/health` → `"version": "11.5.0-pre"` (with `"commit": "f3d1ae504a"`).
- `GET /api/frontend/settings` → `buildInfo.version = "11.5.0-pre"` and
  `buildInfo.versionString = "Grafana v11.5.0-pre (f3d1ae504a)"`.
...
section were captured from a binary built at branch HEAD `f3d1ae504a`
(`f3d1ae504a5cc9568a40f921a070e2fe165ddb6f`); a binary linked at a different HEAD would report the
...
buildInfo.versionString = 'Grafana v11.5.0-pre (f3d1ae504a)'
buildInfo.commit = 'f3d1ae504a'
buildInfo.commitShort = 'f3d1ae504a'
buildInfo.buildstamp = 1784019135
